...
class Item { 
	...
	int idx; 
	public Item() {} 
	public Item(int value, long weight, 
				int idx) 
	{ 
		this.value = value; 
		this.weight = weight; 
...
class BranchAndBound { 
	...
	static float upperBound(float tv, long tw, 
							int idx, Item arr[]) 
	{ 
		float value = tv; 
		long weight = tw; 
	...
	static float lowerBound(float tv, long tw, 
							int idx, Item arr[]) 
	{ 
		float value = tv; 
		long weight = tw; 
	...
	static void assign(Node a, long ub, long lb, 
	...
	throws IOException{ 
		int n = readInt(); long w = readInt();
		Item [] values = new Item [n]; 
		for (int i = 0; i< n;i++) {
			values[i] = new Item(readInt(), readInt(), i);
		}
